...
public class BukkitChatPlatform extends ChatPlatformBuilder {
    ...
    public static <T> ChatInput.Builder<T, Player> builder(@NotNull final ChatPlatform<Player> platform,
                                                           @NotNull final Player sender) {
        return PaperChatPlatform.builder(platform, new BktChatSender(sender));
    }
    ...
    public ChatTask createRunTaskLater(@NotNull final Runnable runnable, final long time) {
        return new BktChatTask(Bukkit.getScheduler().runTaskLater(this.plugin, runnable, time));
    }
    ...
    public void whenChat(@NotNull final AsyncPlayerChatEvent event) {
        this.input.get().onChat(new BktChatEvent(event, new BktChatSender(event.getPlayer())));
    }
    ...
    public void whenQuit(@NotNull final PlayerQuitEvent event) {
        this.input.get().onQuit(new BktQuitEvent(new BktChatSender(event.getPlayer())));
    }
}
